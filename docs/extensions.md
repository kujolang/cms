...
Plugin manifests may also declare bounded, secret-free `abilities` and `connectors`. Active contributions are merged into `GET /v1/abilities`, `GET /v1/ai/connectors`, and MCP-ready tool discovery. Their raw package descriptors remain discoverable at `GET /v1/extensions/ai` and `bash scripts/cms-extensions.sh ai`. CMS executes an enabled plugin ability through the active plugin's configured `service_url`, its declared method and path, the standard permission check, confirmation rules, outbound URL policy, a bounded timeout, and a 1 MB response ceiling. Set a runtime bearer outside the database as `CMS_PLUGIN_<NORMALIZED_PLUGIN_KEY>_BEARER`; it is never returned by discovery APIs. Private-network runtimes remain blocked by default; self-hosted operators must explicitly set `CMS_PLUGIN_RUNTIME_ALLOW_PRIVATE=true` and a narrow `CMS_PLUGIN_HOOK_URL_ALLOWLIST`. Ability and connector states can be controlled independently from plugin activation, and connector health is available through `POST /v1/ai/connectors/:key/health` or `bash scripts/cms-ai.sh connector:health <key>`.
