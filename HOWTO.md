...
Treat the editorial studio and its server proxy as a protected application, even when the public delivery API remains anonymous. Require an authenticated identity for every studio page and API request, keep sessions in signed `HttpOnly` and `SameSite` cookies (or use the hosting platform's authenticated identity headers), apply same-origin checks to mutations, rate-limit sign-in attempts, and enforce capabilities such as content editing, publishing, taxonomy management, SEO management, media upload, and user administration on the server. UI visibility is helpful feedback, but it is not authorization.

Populate author controls from the trusted user directory rather than accepting arbitrary browser text. A human account's role and capabilities belong in the trusted authentication layer; the CMS bearer token remains a separate server-to-server credential. Custom taxonomies can be created through authenticated `POST /v1/taxonomies` requests and their values through `POST /v1/taxonomies/:id/terms`.

For a complete account system, persist users through `/v1/users` rather than embedding them in frontend configuration. Keep profiles, email, username, biography, website, avatar, social links, role, account status, approval metadata, and derived password credentials in the backend. Let administrators create and edit users through bearer-protected routes, expose self-service profile changes through an application server that restricts updates to the signed-in user, and choose a registration policy through `/v1/settings/registration`: `open` activates new accounts immediately, `approval` creates pending accounts for review, and `closed` limits account creation to administrators. New public accounts should default to a low-privilege role such as `subscriber`; registration must never grant editorial capabilities by default.
...
- Put the API behind a trusted ingress that enforces the configured CMS request-body size and set `CMS_TRUSTED_INGRESS_LIMITS=true`. Use `CMS_RATE_LIMIT_MODE=sqlite` for durable local limits keyed by Kujo's socket-derived peer IP, or `external` for shared ingress limits. Kujo enforces the socket read deadline through `KUJO_HTTP_SERVER_READ_TIMEOUT_MS`.
